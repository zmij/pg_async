...
BOOST_AUTO_TEST_CASE(WriteParamsTest)
{
	using namespace tip::db::pg;
	{
        ...
        BOOST_CHECK_EQUAL(buffer.size(), sizeof(smallint) * 2 + sizeof(integer) + sizeof(smallint));
        
        buffer.clear();
        tip::db::pg::detail::write_params(buffer, (integer)42);
        BOOST_CHECK(!buffer.empty());
        BOOST_CHECK_EQUAL(buffer.size(), sizeof(smallint) * 2 + sizeof(integer) + sizeof(integer));
        
        buffer.clear();
        tip::db::pg::detail::write_params(buffer, (integer)42, (smallint)324);
        BOOST_CHECK(!buffer.empty());
        BOOST_CHECK_EQUAL(buffer.size(), sizeof(smallint) * 2 + sizeof(integer) + sizeof(integer) + sizeof(integer) + sizeof(smallint));
	...
	if (!test_database.empty()) {
		...
		connection_options opts = connection_options::parse(test_database);
		{
			...
			[&](connection_lock_ptr c, resultset, bool){
				local_log() << "Query one finished";
				BOOST_CHECK(c);
				query(c, "insert into pg_async_test values($1),($2),($3)")
				.bind( (bigint)12345, (bigint)987656, (bigint)12389754876 ).run_async(
				[&](connection_lock_ptr c, resultset, bool){
					#ifdef WITH_TIP_LOG
					local_log() << "Query two finished";
					#endif
