...
query::query(dbalias const& alias, std::string const& expression,
        T const& ... params)
    : pimpl_(create_impl(alias, transaction_mode{}, expression, params ...))
{
}
...
query::query(dbalias const& alias, transaction_mode const& mode,
    ...
    : pimpl_(create_impl(alias, mode, expression, params ...))
{
}
        ...
        std::string const& expression, T const& ... params)
{
    ...
    return create_impl(alias, mode, expression, std::move(ptypes), std::move(buf));
}
